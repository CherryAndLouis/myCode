...
class web_spider():
    ...
    def __init__(self):
        self.url = self.get_conf_para()["swagger_url"]+"/v2/api-docs"
    ...
    def get_http_jmx(self,interface_list,control_element,http_element,assert_element):
        add_url_list = self.get_conf_para()["swagger_url"].split(":")[2].split('/')
        add_url = ''
        if len(add_url_list) > 2:
            for i in range(1,len(add_url_list)):
                if add_url_list[i] != '':
                    add_url = add_url + '/' + add_url_list[i]
        ...
        for interface_dic in interface_list:
            ...
            for interface_url in interface_dic.keys():
                request_url = add_url + interface_url[:interface_url.find(',')]
    ...
    def generate_jmx(self,para_node,http_node,jmx_name):
        ...
        # 设置http默认配置参数
        default_http_element = dom_tree.getElementsByTagName("ConfigTestElement")[0]
        host_element = default_http_element.getElementsByTagName("stringProp")[0]
        # 获得IP和端口
        url_port = self.url.split('//')[1].split('/')[0]
        host_element.firstChild.data = url_port[:url_port.find(':')]
        port_element = default_http_element.getElementsByTagName("stringProp")[1]
        port_element.firstChild.data = url_port[url_port.find(':')+1:]
        protocol_element = default_http_element.getElementsByTagName("stringProp")[2]
        protocol_element.firstChild.data = 'http'
        ecoding_element = default_http_element.getElementsByTagName("stringProp")[3]
        ecoding_element.firstChild.data = 'utf-8'
        pool_element = default_http_element.getElementsByTagName("stringProp")[5]
        pool_element.firstChild.data = '4'
        implementation_element = default_http_element.getElementsByTagName("stringProp")[6]
        implementation_element.firstChild.data = 'HttpClient4'
        connect_timeout_element = default_http_element.getElementsByTagName("stringProp")[7]
        connect_timeout_element.firstChild.data = '10000'
        response_timeout_element = default_http_element.getElementsByTagName("stringProp")[8]
        response_timeout_element.firstChild.data = '10000'
        # 设置用户参数
...
if __name__ == '__main__':
    ...
    print("jmx生成完成，请进入result文件夹查看结果！")
